...
import { formatINR, formatQuantity, getUnitsForDimension } from '@/lib/utils/units'
...
export default function SellerProductsPage() {
  ...
  function updateSelection(productId: string, field: string, value: string) {
    ...
    if (field === 'quantity') {
      ...
      updated.unit = value
      // Recalculate price for new unit
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-900 mt-4 mb-6">Browse Products</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-800">
              <strong>{Object.keys(selectedProducts).length}</strong> product(s) selected
            </div>
          </div>

          {/* Products Grid and Order Summary */}
          <div className="lg:col-span-2">
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">
                      Units: {product.available_units.join(', ')}
                    </p>
                      ...
                      <div className="mt-4 pt-4 border-t space-y-3" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="bg-green-50 p-2 rounded">
                          <p className="text-sm text-green-800">
                            <strong>Price:</strong> {formatINR(
                              new Decimal(selection.quantity).times(selection.pricePerUnit)
                            )}
                          </p>
                        </div>
                      </div>
            ...
            {Object.keys(selectedProducts).length > 0 && (
              <div className="card border-2 border-green-200 bg-green-50">
                    ...
                    <span>
                      {formatINR(
                        Object.values(selectedProducts).reduce(
                          (sum, sel) => sum.plus(new Decimal(sel.quantity).times(sel.pricePerUnit)),
                          new Decimal(0)
                        )
                      )}
                    </span>
                ...
                >
                  {submitting ? 'Placing Order...' : 'Place Order / Quotation'}
                </button>
              </div>
            )}
          </div>
